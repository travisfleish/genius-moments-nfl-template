...
// ─────────────────────────────────────────────────────────────────────────────
// MOMENTS — SPAIN v FRANCE · WORLD CUP SEMI-FINAL
// Spain 2–0 win, visualized as the Moments Engine would read it:
...
//
// ⚠ MOCK DATA — win-probability curve is directional, redrawn from a Genius IQ
// "home win probability" (France) chart screenshot and inverted to read as
// Spain's match-outcome win prob. Goal times/scorers are real.
//   MATCH  → competition / team names / final score copy
//   GOALS  → goal timeline (game-progress p, scoring team)
//   KEYS   → Spain win-probability curve (sharp step at each goal)
//   EMO    → Spain-fan emotional arc
...
const MATCH = {
  ...
  usaName: 'Spain', belName: 'France',
  finalUSA: 2, finalBEL: 0,
};
...
// ── Goal timeline — real Spain v France events (game-progress p = min/100) ──
const GOALS = [
  [m2p(22), 'usa'],   // 22'   Mikel Oyarzabal (pen) — Spain 1–0
  [m2p(58), 'usa'],   // 58'   Pedro Porro            — Spain 2–0
];
...
// ── Spain (away) win-probability curve — derived from a Genius IQ home-win-prob chart ──
// (France's home win prob crashes from ~47% to ~1% across the two Spain goals;
// this is that curve inverted to read as Spain's match-outcome win prob.)
const KEYS = [
  [0.00, .53], [0.20, .53],                 // pre-match → close to a coin-flip
  [0.22, .78],                              // 22' Oyarzabal pen. → Spain 1–0, sharp step up
  [0.30, .79], [0.40, .81], [0.45, .82],
  [0.50, .84], [0.55, .86], [0.58, .87],    // held lead through to 58'
  [0.582, .96],                             // 58' Porro → Spain 2–0, second sharp step up
  [0.60, .98],
  [0.70, .982], [0.85, .985], [1, .99],
];
function baseWP(p) {
...
const camTimes = [0, 4, 8, 9.65, 11.2, 16.43, 18.0, 24.35, 24.75, 25.95, 27.15, 33.9];
const camS  = interpolate(camTimes, [1, 1, 1.04, 1.16, 1.18, 1.22, 1.24, 1.24, 1.30, 1.30, 1, 1], Easing.easeInOutCubic);
const camFX = interpolate(camTimes, [960, 960, 960, 700, 750, 1150, 1200, 1500, 1560, 1560, 960, 960], Easing.easeInOutCubic);
const camFY = interpolate(camTimes, [540, 540, 540, 560, 565, 580, 585, 590, 595, 595, 540, 540], Easing.easeInOutCubic);

// ── Emotion arc (Spain-fan perspective) ────────────────────────────────────────
// Emotions are the manual's canonical set only (Joy · Awe & Admiration · Hope · Tension · Sadness · Relief).
const EMO = [
  ['Hope',             0.00, 0.22, C.lightBlue],    // kickoff — belief / anticipation
  ['Joy',              0.22, 0.45, C.lightGreen],   // 22' Oyarzabal pen. — Spain 1–0
  ['Tension',          0.45, 0.58, C.orange],       // still just 1–0, waiting on France's response
  ['Awe & Admiration', 0.58, 0.70, C.brightGreen],  // 58' Porro's clincher — moment of brilliance
  ['Relief',           0.70, 1.0,  C.green],        // game managed out to full time
];
...
function TeamBadge({ team, size = 44 }) {
  const isEsp = team === 'usa';
  const flagStyle = isEsp
...
function ScreenLabel() {
  ...
  React.useEffect(() => {
    ...
    if (el) el.setAttribute('data-screen-label', `moments-spain-france · t=${s}s`);
  }, [s]);
  return null;
}
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 36, textAlign: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ fontFamily: FH, fontWeight: 300, fontSize: 84, letterSpacing: '-0.04em', lineHeight: 1.08, color: C.white, opacity: h2o, transform: `translateY(${(1 - h2o) * 24}px)` }}>
              World Cup Semi-Final: Spain v <span style={{ color: C.lightBlue }}>France</span>
            </div>
          </div>
...
function ChartFrame() {
  ...
  return (
    <svg width={W} height={H} style={{ position: 'absolute', inset: 0, opacity: o }}>
      ...
      <text x={CH.x} y={CH.y - 30} fill="rgba(225,255,103,0.85)" fontFamily={FB} fontSize="16" fontWeight="500" letterSpacing="2.5">WIN PROBABILITY — SPAIN</text>
...
function Callouts() {
  return (
    <React.Fragment>
      <Callout t0={tAt(0.24)} tEnd={tAt(0.56)} x={620} y={470} w={340}
        anchorX={X(0.35)} anchorY={Y(wpAt(0.35))} connFrom="bottom"
        title={"Spain lead\n(1–0)"} status="MOMENT ACTIVATED · SPAIN-FAN CREATIVE ON" on={true} />
      <Callout t0={tAt(0.60)} tEnd={DRAW1 + 0.45} x={1050} y={430} w={360}
        anchorX={X(0.65)} anchorY={Y(wpAt(0.65))} connFrom="bottom"
        title={"Spain doubles\nthe lead"} status="MOMENT ACTIVATED · SPAIN CREATIVE ESCALATED" on={true} />
      <Callout t0={DRAW1 + 0.1} x={1372} y={318} w={300} big={true} badge="usa"
        anchorX={X(1)} anchorY={Y(1)} connFrom="top"
        title="Spain win" status="FULL TIME · MOMENT ACTIVATED" on={true} />
      <Callout t0={DRAW1 + 0.55} x={1372} y={470} w={300}
        anchorX={X(0.99)} anchorY={Y(wpAt(0.99))} connFrom="right"
        title="Into the final" status="MOMENT ACTIVATED" on={true} />
    </React.Fragment>
  );
}
...
function AudienceStrip() {
  const t = useTime();
  const p = gameP(t);
  const belState = p >= 0.58 ? 'PAUSED' : 'MONITORING';
  const belChange = tAt(0.58);
  const usaState = p >= 1 ? 'DELIVERED' : p >= 0.22 ? 'DELIVERING' : 'MONITORING';
  return (
    <div style={{ position: 'absolute', left: CH.x, top: 246, width: CH.w, display: 'flex', gap: 36 }}>
      <AudienceCard team="bel" fan="FRANCE FANS" creative="“Stay in it” creative" state={belState} tChange={belChange} />
      <AudienceCard team="usa" fan="SPAIN FANS" creative="“La Roja surge” creative" state={usaState === 'DELIVERED' ? 'DELIVERING' : usaState} tChange={tAt(0.22)} />
    </div>
  );
}
...
const MOMENTS = [
  { p: m2p(22), kind: 'on',  team: 'usa', title: 'Spain strike first',   status: 'MOMENT ACTIVATED · SPAIN-FAN CREATIVE ON',    box: { x: 1160, y: 470 }, t0: tAt(m2p(22)) + 0.2,  t1: tAt(m2p(22)) + 4.6 },
  { p: m2p(58), kind: 'off', team: 'bel', title: 'France’s hopes fade',  status: 'MOMENT DEACTIVATED · FRANCE CREATIVE OFF',    box: { x: 120,  y: 352 }, t0: tAt(m2p(58)) + 0.15, t1: DRAW1 + 0.7 },
  { p: m2p(58), kind: 'on',  team: 'usa', title: 'Spain closing it out', status: 'MOMENT ACTIVATED · SPAIN CREATIVE ESCALATED', box: { x: 120,  y: 528 }, t0: tAt(m2p(58)) + 0.35, t1: DRAW1 + 1.9 },
  { p: 1.0,     kind: 'win', team: 'usa', title: 'Spain win 2–0',        status: 'FULL TIME · MOMENT ACTIVATED',                box: { x: 120,  y: 704 }, t0: DRAW1 + 0.35, t1: 99, big: true },
];
...
const EV = [
  { p: m2p(22), team: 'usa', type: 'goal', label: 'GOAL · PEN', player: 'Mikel Oyarzabal', min: "22'", score: '1–0', box: { x: 110,  y: 352 }, t0: tAt(m2p(22)) - 0.05, t1: tAt(m2p(58)) - 0.1 },
  { p: m2p(58), team: 'usa', type: 'goal', label: 'GOAL',       player: 'Pedro Porro',     min: "58'", score: '2–0', box: { x: 1500, y: 352 }, t0: tAt(m2p(58)) - 0.05, t1: 99 },
];
...
// ROOT
// ─────────────────────────────────────────────────────────────────────────────
function MomentsFilmFranceSpain() {
  return (
    <Stage width={W} height={H} duration={DUR} background={C.navy} persistKey="moments-g5" loop={true}>
      <IntroScene />
      <ChartScene />
      <EndScene />
      <ScreenLabel />
...
window.MomentsFilmFranceSpain = MomentsFilmFranceSpain;
