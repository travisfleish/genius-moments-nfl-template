...
// ═════════════════════════════════════════════════════════════════════════════
// GENIUS MOMENTS — NFL FILM TEMPLATE · 4:5 PORTRAIT CUT
//
// Same data file as the 16:9 cut (nfl-game.js). Only layout and pacing differ:
// 27s instead of 35s, cards stacked into fixed lanes, no camera move.
//
...
const W = 1080, H = 1350, DUR = 27;
const DRAW0 = 5.0, SPAN = 14.0;                 // game draws continuously, kickoff → final
const DRAW1 = DRAW0 + SPAN;                     // 25.4 — final whistle, win beat
const MXX = 56;
const CH = { x: MXX, y: 580, w: W - MXX * 2, h: 470 };
...
function buildTimeline(src) {
  ...
  const moments = (game.moments || []).map((m) => {
    ...
    const t1 = isWin ? 99 : Math.min(DRAW1 + 0.6, t0 + 4.2);
    let box;
    if (isWin) { box = { x: 0, y: 0 }; }
    else {
  ...
  const events = scoring.filter((s) => s.feature).map((s, i) => Object.assign({}, s, {
    t0: tAtP(s.p) - 0.05,
    t1: Math.min(DRAW1 + 0.4, tAtP(s.p) + 3.4),
...
function ScreenLabel() {
  ...
  React.useEffect(() => {
    ...
    if (el) el.setAttribute('data-screen-label', `nfl-moments-4x5 · t=${s}s`);
  }, [s]);
  return null;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// PORTRAIT (4:5) LAYOUT — same data, same story, restacked for feed
// ─────────────────────────────────────────────────────────────────────────────
const BAR_TOP = 258, BAR_H = 46;
const AUD_TOP = 340, AUD_H = 76, AUD_GAP = 12;
const CARD_Y = 1136;

function IntroScene() {
  const t = useTime();
  const o = 1 - fadeIn(t, 3.4, 0.6);
  const g = G.game;
  return (
    <Sprite start={0} end={4.1}>
      <div style={{ position: 'absolute', inset: 0, opacity: o }}>
        <Spring x={80} y={1030} height={110} t0={-1} opacity={0.85} />
        <Spring x={940} y={150} height={110} t0={-1} flip={true} opacity={0.85} />
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, textAlign: 'center', padding: '0 70px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontFamily: FH, fontWeight: 300, fontSize: 62, letterSpacing: '-0.04em', lineHeight: 1.1, color: C.white }}>
              {introParts(g.intro.line1)}
            </div>
            <div style={{ fontFamily: FH, fontWeight: 300, fontSize: 52, letterSpacing: '-0.04em', lineHeight: 1.12, color: 'rgba(255,255,255,0.88)' }}>
              {g.intro.line2}
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 28, marginTop: 24 }}>
            <TeamBadge team="away" size={104} />
            <span style={{ fontFamily: FH, fontWeight: 300, fontSize: 34, color: 'rgba(255,255,255,0.5)' }}>at</span>
            <TeamBadge team="home" size={104} />
          </div>
        </div>
      </div>
    </Sprite>
...
function Header() {
  const t = useTime();
  const o = fadeIn(t, 3.7, 0.5);
  const p = gameP(t);
  const g = G.game;
  const live = t >= DRAW0;
  const ck = fmtClock(p * G.total, g);
  const state = !live ? 'KICKOFF' : p >= 1 ? 'FINAL' : `${ck.period}  ${ck.clock}`;
  const sc = scoreAt(p);
  return (
    <div style={{ position: 'absolute', left: MXX, width: W - MXX * 2, top: 46, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, opacity: o, transform: `translateY(${(1 - o) * -12}px)` }}>
      <div style={{ fontFamily: FB, fontSize: 13, fontWeight: 500, letterSpacing: '0.18em', color: 'rgba(255,255,255,0.55)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 26 }}>
        ...
        <div style={{ fontFamily: FH, fontWeight: 700, fontSize: 46, letterSpacing: '-0.02em', color: C.white, fontVariantNumeric: 'tabular-nums' }}>
          {sc.away} <span style={{ color: 'rgba(255,255,255,0.3)', fontWeight: 300 }}>–</span> {sc.home}
        </div>
        <TeamBadge team="home" size={56} />
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
        <span style={{ fontFamily: FH, fontWeight: 700, fontSize: 22, letterSpacing: '-0.02em', color: p >= 1 && live ? C.brightGreen : 'rgba(255,255,255,0.85)', fontVariantNumeric: 'tabular-nums' }}>{state}</span>
        <span style={{ width: 1, height: 18, background: 'rgba(255,255,255,0.2)' }} />
        <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <Pulse size={8} />
          <span style={{ fontFamily: FB, fontSize: 13, fontWeight: 500, letterSpacing: '0.16em', color: 'rgba(255,255,255,0.8)' }}>MOMENTS ENGINE · LIVE</span>
        </span>
      </div>
    </div>
  );
...
function EmotionBar() {
  const t = useTime();
  const p = gameP(t);
  const capO = fadeIn(t, 3.8, 0.5);
  const live = t >= DRAW0;
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      <div style={{ position: 'absolute', left: MXX, top: BAR_TOP - 26, fontFamily: FB, fontSize: 12, fontWeight: 500, letterSpacing: '0.14em', color: 'rgba(255,255,255,0.5)', opacity: capO }}>
        {persp().name.toUpperCase()}-FAN EMOTIONAL ARC
      </div>
      {G.emotion.map((seg, i) => {
        const entry = fadeIn(t, 3.8 + i * 0.05, 0.4);
        ...
        const label = width < 38 ? '' : width < 96 ? seg.label.split(' ')[0] : seg.label;
        const base = {
          ...
          fontFamily: FH, fontWeight: 400, fontSize: width < 96 ? 11.5 : 13.5, letterSpacing: '-0.01em', lineHeight: 1.1,
      ...
      {live && p < 1 && (
        <div style={{ position: 'absolute', left: X(p) - 6, top: BAR_TOP + BAR_H + 5, width: 0, height: 0, borderLeft: '6px solid transparent', borderRight: '6px solid transparent', borderBottom: `8px solid ${C.brightGreen}` }} />
      )}
    </div>
  );
}

function AudienceCard({ a, index }) {
  const t = useTime();
  const p = gameP(t);
  const o = fadeIn(t, 4.3 + index * 0.08, 0.5);
  const state = p >= a.offP ? 'PAUSED' : p >= a.onP ? 'DELIVERING' : 'MONITORING';
  const pop = 1 + 0.015 * decay(t, tAt(p >= a.offP ? a.offP : a.onP), 4);
  const isOn = state === 'DELIVERING';
  return (
    <div style={{
      width: '100%', height: AUD_H, borderRadius: 14, boxSizing: 'border-box',
      ...
      <TeamBadge team={a.team} size={40} />
      <Pulse size={10} dim={!isOn} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: FB, fontSize: 12.5, fontWeight: 500, letterSpacing: '0.16em', color: 'rgba(255,255,255,0.55)' }}>{a.label}</div>
        <div style={{ fontFamily: FB, fontSize: 18, fontWeight: 500, letterSpacing: '-0.01em', color: 'rgba(255,255,255,0.92)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.creative}</div>
      </div>
      <div style={{ fontFamily: FB, fontSize: 13, fontWeight: 500, letterSpacing: '0.14em', flexShrink: 0, color: isOn ? C.brightGreen : 'rgba(255,255,255,0.4)' }}>{state}</div>
    </div>
  );
}
...
function AudienceStrip() {
  return (
    <div style={{ position: 'absolute', left: MXX, top: AUD_TOP, width: W - MXX * 2, display: 'flex', flexDirection: 'column', gap: AUD_GAP }}>
      {G.audiences.map((a, i) => <AudienceCard key={i} a={a} index={i} />)}
    </div>
  );
}
...
function ChartFrame() {
  ...
  return (
    <svg width={W} height={H} style={{ position: 'absolute', inset: 0, opacity: o }}>
      ...
      <text x={CH.x} y={CH.y - 22} fontFamily={FB} fontSize="14" fontWeight="500" letterSpacing="2.2">
      ...
      {G.periods.slice(1).map((q) => {
        const strong = q.label === 'Q3';
        const pulse = decay(t, tAt(q.p0), 2.5);
        return <line key={q.label} x1={X(q.p0)} y1={CH.y} x2={X(q.p0)} y2={CH.y + CH.h}
          stroke={`rgba(255,255,255,${(strong ? 0.2 : 0.09) + pulse * 0.4})`} strokeWidth={(strong ? 1.4 : 1) + pulse * 1.4} />;
      })}
      {G.scoring.filter((s) => s.leadChange).map((s, i) => {
        ...
        return (
          <g key={`lc${i}`}>
            ...
            <text x={X(s.p)} y={Y(0.5) - 17} textAnchor="middle" fill={C.white} fontFamily={FB} fontSize="11.5" fontWeight="500" letterSpacing="1.6" opacity={pulse * 1.6}>LEAD CHANGE</text>
          </g>
        );
      })}
      {G.scoring.map((s, i) => {
        ...
        return (
          <g key={i}>
            ...
            <circle cx={X(s.p)} cy={CH.y - 9} r={3.5 + pulse * 3} fill={col} opacity={0.9} />
            <text x={X(s.p)} y={CH.y + CH.h + (i % 2 ? 48 : 30)} textAnchor="middle" fill={col} fontFamily={FB} fontSize="12" fontWeight="500" letterSpacing="1.2" opacity={0.85}>
              {`${tm.abbr} ${s.type}`}
            </text>
          </g>
        );
      ...
      {[[1, '1.0'], [0.5, '0.5'], [0, '0.0']].map(([v, l]) => (
        <text key={l} x={CH.x + 10} y={clamp(Y(v) - 8, CH.y + 16, CH.y + CH.h - 8)} fill="rgba(255,255,255,0.35)" fontFamily={FB} fontSize="14">{l}</text>
      ))}
      {G.periods.map((q) => (
        <text key={q.label} x={(X(q.p0) + X(q.p1)) / 2} y={CH.y + CH.h + 72} textAnchor="middle"
          fill={q.label === 'OT' ? 'rgba(225,255,103,0.8)' : 'rgba(255,255,255,0.5)'} fontFamily={FB} fontSize="16" letterSpacing="2">{q.label}</text>
      ))}
    </svg>
  );
}
...
function WinLine() {
  ...
  return (
    <svg width={W} height={H} style={{ position: 'absolute', inset: 0 }}>
      ...
      <path d={d} fill="none" stroke={C.brightGreen} strokeWidth="2.8" strokeLinejoin="round" strokeLinecap="round" />
      {p < 1 && <circle cx={tipX} cy={tipY} r={8 + k * 15} fill="none" stroke={C.brightGreen} strokeWidth="2" opacity={(1 - k) * 0.55} />}
      <circle cx={tipX} cy={tipY} r="6" fill={C.brightGreen} />
      <text x={Math.min(tipX + 12, CH.x + CH.w - 54)} y={clamp(tipY - 18, CH.y + 18, CH.y + CH.h - 8)} fill={C.brightGreen} fontFamily={FB} fontSize="19" fontWeight="500">{Math.round(wpAt(p) * 100)}%</text>
    </svg>
  );
}
...
function WinFlash() {
  ...
  const k = t - DRAW1;
  const flash = interpolate([0, 0.12, 0.8], [0, 0.36, 0])(k);
  return (
    <React.Fragment>
...
// Scoring-play ticker — a compact chip that rides above its marker on the chart
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {active.map((ev, i) => {
        ...
        const o = fadeIn(t, ev.t0, 0.28) * (ev.t1 < 90 ? 1 - fadeIn(t, ev.t1 - 0.35, 0.35) : 1);
        const accent = ev.team === (G.game.perspective || 'home') ? C.brightGreen : C.coral;
        const cx = clamp(X(ev.p), CH.x + cardW / 2, CH.x + CH.w - cardW / 2);
        return (
          <div key={i} style={{ position: 'absolute', left: cx - cardW / 2, top: CH.y + 20, width: cardW, opacity: o,
            transform: `scale(${0.75 + 0.25 * e})`, transformOrigin: 'center top',
            borderRadius: 12, padding: '12px 16px', boxSizing: 'border-box',
            background: 'rgba(13,18,38,0.86)', border: '1px solid rgba(255,255,255,0.14)', borderLeft: `3px solid ${accent}`,
            backdropFilter: 'blur(4px)', display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontFamily: FH, fontWeight: 400, fontSize: 20, letterSpacing: '-0.01em', color: C.white, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ev.player}</span>
              <span style={{ fontFamily: FH, fontWeight: 700, fontSize: 16, color: 'rgba(255,255,255,0.75)', marginLeft: 'auto', flexShrink: 0, whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{ev.away}–{ev.home}</span>
            </div>
            {ev.drive && <div style={{ fontFamily: FB, fontSize: 12.5, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.04em' }}>{ev.drive}</div>}
          </div>
        );
      })}
    </div>
...
// Creative activation — one full-width spotlight card at a time, below the chart
function SpotlightCards() {
  const t = useTime();
  const active = G.moments.filter((m) => t >= m.t0 && t <= m.t1 && !m.big);
  const win = G.moments.filter((m) => m.big && t >= m.t0 && t <= m.t1);
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {active.map((m, i) => {
        const e = popIn(t, m.t0, 0.5);
        const o = fadeIn(t, m.t0, 0.3) * (m.t1 < 90 ? 1 - fadeIn(t, m.t1 - 0.35, 0.35) : 1);
        const isOn = m.kind === 'on';
        return (
          <div key={i} style={{
            position: 'absolute', left: MXX, top: CARD_Y, width: W - MXX * 2, boxSizing: 'border-box',
            borderRadius: 14, padding: '20px 24px', opacity: o,
            transform: `scale(${0.72 + 0.28 * e})`, transformOrigin: 'center',
            display: 'flex', alignItems: 'center', gap: 18,
            ...(isOn
              ? { background: C.brightGreen, color: C.navy, border: 'none', boxShadow: '0 0 60px rgba(225,255,103,0.3)' }
              : { background: 'rgba(13,18,38,0.82)', color: C.white, border: `1.5px solid ${C.coral}`, backdropFilter: 'blur(4px)', boxShadow: '0 0 44px rgba(247,107,106,0.2)' }),
          }}>
            <TeamBadge team={m.team} size={46} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7, minWidth: 0 }}>
              <span style={{ fontFamily: FH, fontWeight: 400, fontSize: 30, letterSpacing: '-0.02em', lineHeight: 1.12 }}>{m.title}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, background: isOn ? C.navy : C.coral }} />
                <span style={{ fontFamily: FB, fontSize: 13.5, fontWeight: 500, letterSpacing: '0.1em', opacity: isOn ? 0.78 : 0.72 }}>{m.status}</span>
              </span>
            </div>
          </div>
        );
      ...
      {win.map((m, i) => {
        const e = popIn(t, m.t0, 0.5);
        const o = fadeIn(t, m.t0, 0.3);
        return (
          <div key={`w${i}`} style={{
            position: 'absolute', left: MXX + 40, top: CH.y + 150, width: W - MXX * 2 - 80, boxSizing: 'border-box',
            borderRadius: 14, padding: '26px 28px', opacity: o,
            transform: `scale(${0.62 + 0.38 * e})`, transformOrigin: 'center',
            background: C.brightGreen, color: C.navy, boxShadow: '0 0 70px rgba(225,255,103,0.45)',
            display: 'flex', flexDirection: 'column', gap: 12,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <span style={{ fontFamily: FH, fontWeight: 700, fontSize: 38, letterSpacing: '-0.03em', lineHeight: 1.08 }}>{m.title}</span>
              <TeamBadge team={m.team} size={46} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
              <span style={{ width: 9, height: 9, borderRadius: '50%', background: C.navy }} />
              <span style={{ fontFamily: FB, fontSize: 14, fontWeight: 500, letterSpacing: '0.1em', opacity: 0.78 }}>{m.status}</span>
            </div>
          </div>
        );
      })}
...
function SwingCard() {
  ...
  return (
    <div style={{
      position: 'absolute', left: MXX, top: CARD_Y, width: W - MXX * 2, boxSizing: 'border-box',
      borderRadius: 14, opacity: o, pointerEvents: 'none',
      background: 'rgba(13,18,38,0.86)', border: '1px solid rgba(225,255,103,0.4)', backdropFilter: 'blur(4px)',
      padding: '18px 24px', display: 'flex', alignItems: 'center', gap: 22,
      transform: `scale(${0.74 + 0.26 * e})`, transformOrigin: 'center',
    }}>
      <span style={{ fontFamily: FH, fontWeight: 300, fontSize: 60, letterSpacing: '-0.04em', color: C.brightGreen, lineHeight: 1, flexShrink: 0 }}>
        {s.delta >= 0 ? '+' : '−'}{pts}
      </span>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, minWidth: 0 }}>
        <div style={{ fontFamily: FB, fontSize: 12.5, fontWeight: 500, letterSpacing: '0.16em', color: 'rgba(255,255,255,0.55)' }}>BIGGEST SWING OF THE GAME</div>
...
function ChartScene() {
  const t = useTime();
  const o = fadeIn(t, 3.6, 0.6) * (1 - fadeIn(t, 23.0, 0.8));
  return (
    <Sprite start={3.5} end={24.0}>
      <div style={{ position: 'absolute', inset: 0, opacity: o }}>
        <Header />
        <EmotionBar />
        <AudienceStrip />
        ...
        <img src="assets/logos/GENIUS_SPORTS_HORIZONTAL_WHITE_RGB.svg" alt="Genius Sports"
          style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', bottom: 40, height: 24, opacity: 0.55 }} />
      </div>
    </Sprite>
  );
}
...
function EndScene() {
  const t = useTime();
  const o = fadeIn(t, 23.1, 0.7) * (1 - fadeIn(t, 26.0, 1.0));
  const h1o = fadeIn(t, 23.5, 0.7), subo = fadeIn(t, 23.9, 0.7), logoo = fadeIn(t, 24.4, 0.7);
  const g = G.game;
  return (
    <Sprite start={23.0} end={27}>
      <div style={{ position: 'absolute', inset: 0, opacity: o }}>
        <Spring x={90} y={200} height={110} t0={23.4} flip={true} opacity={0.85} />
        <Spring x={930} y={1000} height={110} t0={23.6} opacity={0.85} />
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, textAlign: 'center', padding: '0 80px' }}>
          <GMTitle t0={23.3} size={44} />
          <div style={{ fontFamily: FH, fontWeight: 300, fontSize: 54, letterSpacing: '-0.03em', lineHeight: 1.1, color: C.white, opacity: h1o, transform: `translateY(${(1 - h1o) * 22}px)` }}>
            {g.end.headline}
          </div>
          <div style={{ fontFamily: FB, fontSize: 26, fontWeight: 400, color: 'rgba(255,255,255,0.72)', letterSpacing: '-0.01em', opacity: subo, transform: `translateY(${(1 - subo) * 16}px)` }}>
            {g.end.sub}
          </div>
          <img src="assets/logos/GENIUS_SPORTS_HORIZONTAL_WHITE_RGB.svg" alt="Genius Sports"
            style={{ height: 66, opacity: logoo * 0.95, transform: `translateY(${(1 - logoo) * 12}px)`, marginTop: 14 }} />
        </div>
      </div>
    </Sprite>
  );
...
function NFLMomentsFilm4x5() {
  syncGameData();
  return (
    <Stage width={W} height={H} duration={DUR} background={C.navy} persistKey="nfl-moments-4x5" loop={true}>
      <IntroScene />
      <ChartScene />
      <EndScene />
      <ScreenLabel />
...
window.NFLMomentsFilm4x5 = NFLMomentsFilm4x5;
